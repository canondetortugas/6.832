...
% create the mesh
% Creates two len(q_bins)xlen(qdot_bins) matrices which together
% form the mesh; q follows q_bins in dimension 1 and is constant in
% dimension 2, whereas qdot is constant in dimension 1 and follows
% qdot_bins in dimension 2. Taking [q(i,j); qdot(i,j)] gives the
% discretized state space at bin (i,j).
[q qdot] = ndgrid(q_bins,qdot_bins);
% Reshape (taking elements column-wise) into a 2x(len(q)*len(qdot)) vector where each column is
% a different state.
...
S = repmat(s,1,na); % repeat s na times - 2x(na*ns) matrix
                    % repeating every ns elements
A = reshape(repmat(a,ns,1),1,ns*na); % repeat a ns times giving a
                                     % 1x(na*ns) matrix where each
                                     % action is repeated ns times
                                     % in a row, followed by the
                                     % next action ns times, and so on...

% compute the one-step dynamics
% Each ns long chunk of columns represents the entire state space
% with a fixed action a performed on it.
...
% LQR cost
Q = diag([100, 10]).*dt; % <== the dt just discretizes
R = 10*dt;
...
% ==============================================================
% Clamps s so that each state is between q/qdot_bins(1) and q/qdot_bins(end)
